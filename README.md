...
Требования для выпуска: Maven на JDK 25, Tycho 5.0.3 и проверенный
материализованный target из точной closure EDT `2026.2.0.289`. Передайте его
каталог `Eclipse` как Maven system property; target definition разрешает
Eclipse, Xtext и `com._1c.g5.v8.*` только из этого каталога:

```bash
mvn -Dtycho.version=5.0.3 -Dedt.home=/path/to/materialized/Eclipse -DskipTests package
```
...
```powershell
mvn -Dtycho.version=5.0.3 "-Dedt.home=C:\path\to\materialized\Eclipse" -DskipTests package
```

`edt.home` не имеет machine-specific default. Не указывайте общий p2 pool или
смешанную инсталляцию: проверяйте marker materialization, hash `bundles.info`
исходной EDT и версии bundle manifests перед сборкой. Выпускной артефакт
собирайте полным reactor из корня репозитория.
...
MCP Host не применяет ограничения профилей встроенного агента, даже если
ранее был задан `sessionProfileId` или системное свойство `sessionProfile`:
эти устаревшие настройки игнорируются для `tools/list` и `tools/call`.
`defaultMutationDecision=ALLOW` разрешает вызов всех
опубликованных инструментов без интерактивного подтверждения, включая
`edt_diagnostics` с `command=metadata_smoke`, `workspace_import_project` и
`extension_manage`. Проверки внутри инструментов продолжают работать:
инструменты, которые требуют `validation_token`, по-прежнему проверяют токен
из `edt_validate_request`; остальные внутренние проверки и валидация параметров
также остаются в силе.
`DENY` отклоняет вызовы; `ASK` тоже отклоняет их, поскольку у MCP Host нет
интерактивного подтверждения. В ChatView список «Роль» над полем ввода
выбирает роль и стиль ответа. Даже при сохранённом `chat.profileId=plan` или
`explore` чат публикует доступные инструменты редактирования BSL и метаданных:
профиль не служит разрешением. Для всех ролей действует единый набор правил
Build с `ASK` для изменений, а также глобальные правила `DENY`/`ASK`,
подтверждения UI, проверки параметров и обязательный `validation_token` для
EDT-мутаций. Выбор роли сохраняется в сессии окна и действует со следующего
хода, включая новый чат в том же окне. Отдельный AgentView и запущенные через
`task` подагенты сохраняют собственные профильные правила; ChatView может
делегировать работу с изменениями, в том числе в `build`. Контекст вызова
сохраняет выбранный ID роли, поэтому внутренние ограничения инструментов,
например пути записи в GSD Ship, остаются активны.

`exposedTools=*` при `ALLOW` и привязке HTTP к loopback (`127.0.0.1`, `::1`
или `localhost`) публикует весь доступный MCP набор, включая инструменты с
метками `sensitive` и `local-exec` (`get_infobase_credentials` и
`java_compile_probe`). При другой привязке или решении `DENY`/`ASK` wildcard
сохраняет исключение этих двух категорий. Явные имена включают инструмент,
а `-имя_инструмента` всегда исключает его; фильтр применяется одинаково к
`tools/list` и `tools/call`. Список также зависит от доступности инструмента
в текущем EDT workspace. Для публикации полного набора оставляйте локальную
привязку и осознанно используйте `ALLOW`. При `authMode=NONE` локальным клиентам
не нужна авторизация; браузерные запросы к `/mcp`, `/llm/v1/chat` и
`/llm/v1/capabilities` с `Origin` принимаются только от loopback origin
(`localhost`, `127.0.0.0/8`, `[::1]`), остальные получают HTTP 403.
Нативные MCP-клиенты без заголовка `Origin` продолжают работать.
