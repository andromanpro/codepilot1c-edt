                ...
                "operations": {
                  ...
                  "items": {
                    ...
                    "properties": {
                      "op": {
                        "type": "string",
                        "enum": ["set_form_props", "add_group", "add_field", "add_table", "add_command", "add_button", "add_handler", "set_item", "set_command", "remove_item", "move_item", "add_event_handler", "set_event_handler", "remove_event_handler"],
                        "description": "Visual form operation. add_field creates a simple UI field; add_table creates a Table item and, with data_path to a ValueTable/ValueTree/tabular attribute, auto-generates its columns. Neither creates the form attribute — create attributes with apply_form_recipe first. add_handler is a legacy alias that only wires a FORM event slot: {op:add_handler, event:OnCreateAtServer, name:ПриСозданииНаСервере}; prefer add_event_handler, which also generates the BSL stub."
                      },
                      "data_path": {
                        "type": "string",
