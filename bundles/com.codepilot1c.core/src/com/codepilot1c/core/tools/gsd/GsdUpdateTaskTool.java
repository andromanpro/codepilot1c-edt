...
public class GsdUpdateTaskTool extends AbstractTool {
    ...
    static final String SCHEMA = """
            {
              ...
              "required": ["project_path", "expected_revision", "task_id", "status"],
    ...
    private ToolResult executeInternal(ToolParameters params) throws IOException {
        ...
        GsdState state = GsdWorkflowService.updateTask(
                projectPath, expectedRevision, taskId, newStatus);
        JsonObject structured = GsdWorkflowService.buildResult(
                true, "gsd_update_task", state.revision(), state.phase(), null); //$NON-NLS-1$
