...
p2_validate() { # <p2_dir> <root_dir> <expect_qualifier> <expect_head> <require_clean_tree:0|1>
  ...
  local q head m_q m_head m_digest
  p2="$1"
  root="$2"
  expect_q="$3"
  expect_head="$4"
  ...
  [ -n "$m_q" ] && [ -n "$m_head" ] && [ -n "$m_digest" ] || p2_fail 16 "unreadable provenance marker: $marker"
  ...
  if [ -n "$expect_head" ] && [ "${head#$expect_head}" = "$head" ]; then
